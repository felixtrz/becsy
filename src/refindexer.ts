...
class Tracker {
	...
	constructor(
		private readonly targetEntityId: EntityId,
		private readonly selector: Selector,
		private readonly trackStale: boolean,
		...
		const precise =
			selector.matchType &&
			((selector.matchSeq &&
				!binding!.fields[selector.sourceSeq!].type.internallyIndexed) ||
				(binding!.refFields.length === 1 &&
					!binding!.refFields[0].type.internallyIndexed));
	...
	private clearRef(sourceId: EntityId, tag: number, final: boolean): void {
		...
		sourceType.__binding!.fields[sourceSeq].clearRef!(
			final,
			this.targetEntityId,
			internalIndex,
		);
	...
	trackReference(
		...
		DEBUG: if (this.clearing) {
			throw new InternalError(
				'Cannot track a new reference while clearing tracker',
			);
		}
	...
	trackUnreference(
		...
		DEBUG: if (index === undefined)
			throw new InternalError('Entity backref not tracked');
		const empty = this.removeTag(
			index,
			this.makeTag(typeId, fieldSeq, internalIndex),
		);
	...
	private removeTag(index: number, tag: number): boolean {
		...
		DEBUG: if (!set.has(tag)) {
			throw new InternalError(
				`Ref ${tag} not tracked (set ${new Array(...set)})`,
			);
		}
		set.delete(tag);
		return !set.size;
	...
	private makeTag(
		...
		return (
			typeId |
			(fieldSeq << COMPONENT_ID_BITS) |
			(internalIndex === undefined
				? 0
				: internalIndex << (COMPONENT_ID_BITS + FIELD_SEQ_BITS))
		);
	}
	...
	private checkUpdateMask(): void {
		...
		for (const targetType of this.selector.targetTypes) {
			if (
				this.registry.hasShape(this.targetEntityId, targetType, this.trackStale)
			) {
...
export class RefIndexer {
	...
	registerSelector(
		targetType?: ComponentType<any>,
		sourceType?: ComponentType<any>,
		sourceFieldSeq?: number,
		trackStale = false,
	): number {
		...
		if (!this.refLog) {
			this.refLog = new Log(
				this.maxRefChangesPerFrame,
				'maxRefChangesPerFrame',
				this.dispatcher.buffers,
		...
		const selectorSourceKey = sourceType
			? sourceFieldSeq === undefined
				? -2 - sourceType.id!
				: sourceType.id! | (sourceFieldSeq << COMPONENT_ID_BITS)
			: -1;
		...
		if (selectorId === undefined) {
			...
			const selector = {
				id: this.selectors.length,
				targetTypes: targetType ? [targetType] : [],
				sourceType,
				matchType: !!sourceType,
				matchSeq: sourceFieldSeq !== undefined,
				sourceTypeId: sourceType?.id,
				sourceSeq: sourceFieldSeq,
				trackStale,
			};
	...
	getBackrefs(entityId: EntityId, selectorId = 0): Entity[] {
		...
		return this.getOrCreateTracker(
			selector,
			entityId,
			this.registry.includeRecentlyDeleted,
		).entities;
	}
	...
	trackRefChange(
		...
		sourceType: ComponentType<any>,
		sourceSeq: number,
		sourceInternalIndex: number | undefined,
		oldTargetId: EntityId,
		newTargetId: EntityId,
		unreference: boolean,
		release: boolean,
	): void {
		DEBUG: if (!this.refLog)
		...
		if (oldTargetId !== -1) {
			const action =
				(unreference ? Action.UNREFERENCE : 0) | (release ? Action.RELEASE : 0);
			DEBUG: if (!action) {
				throw new InternalError(
					'Called trackRefChange with neither unreference nor release',
				);
			}
	...
	private pushRefLogEntry(
		...
		DEBUG: {
			if (
				internallyIndexed &&
				!sourceType.__binding!.fields[sourceSeq].type.internallyIndexed
			) {
		...
		this.refLog!.push(
			targetId |
				(sourceSeq << ENTITY_ID_BITS) |
				action |
				(internallyIndexed ? 2 ** 29 : 0),
		);
	...
	private getOrCreateTracker(
		...
		if (selector.trackStale) {
			...
			this.trackers.set(
				targetId | (selector.id << ENTITY_ID_BITS) | (2 ** 31),
				staleTracker,
			);
		}
	...
	private getTracker(
		...
		return this.trackers.get(
			targetId | (selector.id << ENTITY_ID_BITS) | (stale ? 2 ** 31 : 0),
		);
	}
	...
	flush(): void {
		...
		while (true) {
			...
			for (let i = startIndex!; i < endIndex!; i += 2) {
				...
				const sourceSeq =
					(entryPart2 >>> ENTITY_ID_BITS) & (MAX_NUM_FIELDS - 1);
	...
	private processEntry(
		...
		for (let j = 0; j < this.selectors.length; j++) {
			...
			if (
				(!selector.matchType || selector.sourceTypeId === sourceTypeId) &&
				...
				if (
					selector.trackStale &&
					(action === Action.REFERENCE || action & Action.RELEASE)
				) {
