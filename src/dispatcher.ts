...
import {
	componentTypes as decoratedComponentTypes,
	systemTypes as decoratedSystemTypes,
} from './decorators';
import {
	type Frame,
	FrameImpl,
...
export class Dispatcher {
	...
	constructor({
		...
		const { componentTypes, componentEnums, systemTypes, systemGroups } =
			this.splitDefs([
				defs ?? [],
				decoratedComponentTypes,
				decoratedSystemTypes,
			]);
		if (componentTypes.length > MAX_NUM_COMPONENTS) {
			throw new CheckError(
				`Too many component types, the limit is ${MAX_NUM_COMPONENTS}`,
			);
		}
		...
		this.registry.hasNegativeQueries = this.systems.some(
			(system) => system.hasNegativeQueries,
		);
	...
	private createSystems(
		...
		for (let i = 0; i < systemTypes.length; i++) {
			...
			if (!box) {
				if (!SystemClass.name) {
					Object.defineProperty(SystemClass, 'name', {
	...
	private createSingleGroupFrame(systemTypes: SystemType<System>[]): {
		group: SystemGroup;
		frame: Frame;
	} {
	...
	private createSingletons(): Entity | undefined {
		...
		const singletonComponentDefs = this.systems
			.flatMap((box) => {
				return box.singletonComponentDefs.filter((item, i) => {
					let accepted = true;
					if (typeof item === 'function') {
						accepted =
							i < box.singletonComponentDefs.length - 1 &&
	...
	private splitDefs(defs: DefsArray): {
		...
		return {
			componentTypes,
			componentEnums: Array.from(componentEnums),
			systemTypes,
			systemGroups,
		};
	...
	gatherFrameStats(): void {
		this.stats.frames += 1;
		this.stats.maxShapeChangesPerFrame = this.shapeLog.countSince(
			this.shapeLogFramePointer,
		);
		this.stats.maxWritesPerFrame =
			this.writeLog?.countSince(this.writeLogFramePointer!) ?? 0;
	}
	...
	async terminate(): Promise<void> {
		CHECK: {
			...
			if (this.executingSyncFrame) {
				throw new CheckError(
					'Cannot terminate world from within build callback',
				);
			}
		}
		this.state = State.finish;
	...
	private checkControlOverlap(options: ControlOptions): void {
		...
		for (const def of this.splitDefs(options.restart).systemTypes) {
			...
			if (stopSet.has(def as SystemType<System>)) {
				throw new CheckError(
					`Request to both stop and restart system ${def.name}`,
				);
			}
		}
	}
