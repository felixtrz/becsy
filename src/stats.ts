...
export class SystemStats {
	...
	set lastQueryUpdateDuration(value: number) {
		...
		this.averageQueryUpdateDuration = computeMovingAverage(
			this.averageQueryUpdateDuration,
			value,
		);
	}
	...
	set lastExecutionDuration(value: number) {
		...
		this.averageExecutionDuration = computeMovingAverage(
			this.averageExecutionDuration,
			value,
		);
	}
	...
	set lastCoroutinesDuration(value: number) {
		...
		this.averageCoroutinesDuration = computeMovingAverage(
			this.averageCoroutinesDuration,
			value,
		);
	}
}
...
export class Stats {
	...
	forComponent(type: ComponentType<any>): ComponentStats {
		const componentStats =
			this.components[type.name] ?? new ComponentStats(type);
