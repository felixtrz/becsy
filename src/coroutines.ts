...
class CoroutineImpl<T> implements Coroutine, Waitable<T>, CoroutineGenerator {
	...
	__step(): void {
		...
		try {
			...
			if (
				this.__error &&
				!(this.__awaited || this.__error instanceof CanceledError)
			) {
				throw this.__error;
			}
		} finally {
	...
	cancelIfCoroutineStarted(coroutineFn?: CoroutineFunction): this {
		this.__supervisor.registerCancelIfStarted(
			this,
			this.__scope,
			coroutineFn === coDecorator.self ? this.__fn : coroutineFn,
		);
		return this;
	}
...
function coDecorator(
	target: System,
	name: string,
	descriptor: TypedPropertyDescriptor<CoroutineFunction>,
...
coDecorator.cancelIfComponentMissing = function (
...
coDecorator.cancelIfCoroutineStarted = function (
	coroutineFn?: CoroutineFunction,
): CurrentCoroutine {
	CHECK: checkCurrentCoroutine();
	return currentCoroutine!.cancelIfCoroutineStarted(coroutineFn);
};

coDecorator.self = function* () {
	yield;
};
...
export class Supervisor {
	...
	start<CoFn extends CoroutineFunction>(
		...
		const coroutine = new CoroutineImpl(
			coroutineFn.apply(this.system, args),
			coroutineFn,
			this,
		);
	...
	cancelMatching(
		...
		if (scope) {
			...
			this.cancelMatchingKey(
				startingCoroutine,
				'' + scope.__id + coroutineFn.name,
			);
		}
	}
