...
class VectorType extends Type<number[]> {
	...
	constructor(
		...
		super(
			new Array(typeof elements === 'number' ? elements : elements.length).fill(
				0,
			),
		);
	...
	defineElastic<C>(binding: Binding<C>, field: Field<number[]>): void {
		...
		CHECK: {
			Object.defineProperty(masterWritableAccessor, '__becsyComponent', {
				value: undefined,
				writable: true,
			});
			Object.defineProperty(masterReadonlyAccessor, '__becsyComponent', {
				value: undefined,
				writable: true,
			});
		}
...
class StaticStringType extends Type<string> {
	...
	defineElastic<C>(binding: Binding<C>, field: Field<string>): void {
		...
		const choices = this.choices,
			choicesIndex = this.choicesIndex;
		...
		field.updateBuffer = () => {
			binding.dispatcher.buffers.register(
				...
				binding.capacity,
				this.TypedArray,
				(newData: Uint8Array | Uint16Array | Uint32Array) => {
					data = newData;
				},
			);
		};
...
class DynamicStringType extends Type<string> {
	...
	defineElastic<C>(binding: Binding<C>, field: Field<string>): void {
		...
		field.updateBuffer = () => {
			const size =
				binding.capacity * (this.maxUtf8Length + Uint16Array.BYTES_PER_ELEMENT);
			binding.dispatcher.buffers.register(
				bufferKey,
				size,
				Uint8Array,
		...
		Object.defineProperty(binding.writableMaster, field.name, {
			...
			get(this: C): string {
				...
				return decoder.decode(
					new Uint8Array(
						bytes.buffer,
						binding.writableIndex * bytesStride + 2,
						length,
					),
				);
			},
			set(this: C, value: string): void {
				...
				lengths[binding.writableIndex * lengthsStride] =
					encodedString.byteLength;
		...
		Object.defineProperty(binding.readonlyMaster, field.name, {
			...
			get(this: C): string {
				...
				return decoder.decode(
					new Uint8Array(
						bytes.buffer,
						binding.readonlyIndex * bytesStride + 2,
						length,
					),
				);
			},
	...
	defineElastic<C>(
		...
		Object.defineProperty(binding.writableMaster, field.name, {
			...
			get(this: C): Entity | undefined {
				...
				if (
					id === -1 ||
					(id & STALE_REF_BIT && !registry.includeRecentlyDeleted)
				)
					return;
			...
			set(this: C, value: Entity | undefined | null): void {
				...
				CHECK: if (
					value &&
					!registry.hasShape(value.__id, registry.Alive, false)
				) {
				...
				const stale =
					oldId !== -1 && !!(data[binding.writableIndex] & STALE_REF_BIT);
...
class BackrefsType extends Type<Entity[]> {
	constructor(
		private readonly type?: ComponentType<any>,
	...
	defineElastic<C>(binding: Binding<C>, field: Field<Entity[]>): void {
		...
		const refField = this.fieldName
			? this.type?.__binding!.fields.find(
					(aField) => aField.name === this.fieldName,
			  )
			: undefined;
		CHECK: {
			...
			if (
				this.type &&
				!this.fieldName &&
				!this.type.__binding!.refFields.length
			) {
				throw new CheckError(
					`Backrefs field ${binding.type.name}.${field.name} refers to ` +
		...
		const selectorId = indexer.registerSelector(
			binding.type,
			this.type,
			refField?.seq,
			this.trackDeletedBackrefs,
		);
		...
		Object.defineProperty(binding.writableMaster, field.name, {
			...
			get(this: C): Entity[] {
				...
				CHECK: if (
					...
					binding.dispatcher.registry.includeRecentlyDeleted
				) {
					throw new CheckError(
...
Type.dynamicString = (maxUtf8Length: number) =>
	new DynamicStringType(maxUtf8Length);
Type.ref = new RefType();
Type.backrefs = (
	type?: ComponentType<any>,
	fieldName?: string,
	trackDeletedBackrefs = false,
