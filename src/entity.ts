...
export class EntityImpl {
				...
				if (typeof arg === 'function' && arg.enum) {
					if (enums.has(arg.enum)) {
						throw new CheckError(
							`Can't add multiple components from the same enum`,
						);
					}
					enums.add(arg.enum);
				}
		...
		for (let i = 0; i < args.length; i++) {
			...
			CHECK: {
				if (typeof type !== 'function') {
					throw new CheckError(
						`Bad arguments to addAll: expected component type, got: ${type}`,
					);
				}
			}
	...
	remove(type: ComponentType<any> | ComponentEnum): void {
		...
		if (typeof type !== 'function') {
			...
			CHECK: if (!currentType) {
				throw new CheckError(
					`Entity doesn't have any components from ${type.name} enumeration`,
				);
			}
			type = currentType;
		}
	...
	hasAnyOtherThan(...types: (ComponentType<any> | ComponentEnum)[]): boolean {
		...
		for (const type of this.__registry.types) {
			...
			if (
				!(typeSet.has(type) || (type.enum && typeSet.has(type.enum))) &&
...
export function checkMask(
	...
	if (((mask[binding.shapeOffset] ?? 0) & binding.shapeMask) === 0) {
		throw new CheckError(
			`System ${system.name} didn't mark component ${type.name} as ${kind}able`,
		);
	}
}

export function isMaskFlagSet(
	mask: number[],
	type: ComponentType<any> | ComponentEnum,
): boolean {
	const binding = type.__binding!;
...
export function extendMaskAndSetFlag(
	...
	mask[flagOffset] |= useValues
		? (type as ComponentType<any>).__binding!.shapeValue
		: type.__binding!.shapeMask!;
}
