...
export class Graph<V extends Printable> {
	...
	private setEdge(source: V, target: V, weight: number): void {
		...
		if (
			absWeight < Math.abs(this.edges[sourceToTarget]) ||
	...
	private hasEdgeBetweenIds(sourceId: number, targetId: number): boolean {
		DEBUG: if (sourceId > this.numVertices) {
			throw new InternalError(
				`Vertex id out of range: ${sourceId} > ${this.numVertices}`,
			);
		}
		DEBUG: if (targetId > this.numVertices) {
			throw new InternalError(
				`Vertex id out of range: ${targetId} > ${this.numVertices}`,
			);
		}
		...
		if (cycles.length) {
			...
			throw new CheckError(
				'Precedence cycles detected for the following systems, ' +
					...
					cycles
						.map((cycle) => cycle.map((u) => u.toString()).join('—'))
						.join(', '),
			);
		}
	}
	...
	findCycles(): V[][] {
		...
		const blocked = new Array<boolean>(this.numVertices).fill(false),
			b: Set<number>[] = [];
		const stack: number[] = [],
			cycles: V[][] = [];
