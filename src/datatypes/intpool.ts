...
export class UnsharedPool implements Uint32Pool {
	...
	constructor(
		...
		this.data = new Uint32Array(
			new ArrayBuffer(
				(maxItems + HEADER_LENGTH) * Uint32Array.BYTES_PER_ELEMENT,
			),
		);
	}
	...
	return(id: number): void {
		DEBUG: if (this.length >= this.maxItems) {
			throw new InternalError(
				'Internal error, returned entity ID exceeded pool capacity',
			);
		}
...
export class SharedAtomicPool {
	...
	constructor(
		...
		buffers.register(
			`pool.${configParamName}`,
			maxItems + HEADER_LENGTH,
			Uint32Array,
			(data: Uint32Array) => {
				this.data = data;
			},
