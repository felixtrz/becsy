...
export class Log {
	...
	constructor(
		...
		private readonly options: LogOptions = {
			...
			sortedByComponentType: false,
			numComponentTypes: 0,
		},
	) {
		buffers.register(
			`log.${configParamName}.buffer`,
			maxEntries + HEADER_LENGTH,
			Uint32Array,
			(data: Uint32Array) => {
				this.data = data;
			},
	...
	push(value: number, type?: ComponentType<any>): void {
		...
		DEBUG: if (!!type !== !!this.options.sortedByComponentType) {
			throw new InternalError(
				`Pushing value ${type ? 'with' : 'without'} type to log ${
					this.configParamName
				} ` +
					`${
						this.options.sortedByComponentType ? '' : 'not '
					}sorted by component type`,
			);
		}
	...
	private sortCorral(): Uint32Array {
		...
		if (soleTypeId >= 0) {
			if (soleTypeCount > 1) {
				CHECK: if (this.corral[0] === this.maxEntries)
					this.throwCapacityExceeded();
				this.corral[this.corral[0] + HEADER_LENGTH] =
					this.corral[HEADER_LENGTH];
				this.corral[HEADER_LENGTH] =
					this.corral[0] | (soleTypeId << ENTITY_ID_BITS) | (2 ** 31);
				this.corral[0] += 1;
			}
			this.typeCounters.fill(0);
			return this.corral;
		}
		CHECK: if (this.corral[0] + numNonZeroTypes > this.maxEntries)
			this.throwCapacityExceeded();
	...
	private copyToData(source: Uint32Array): void {
		...
		if (firstSegmentLength < length) {
			this.data.set(
				source.subarray(
					firstSegmentLength + HEADER_LENGTH,
	...
	createPointer(pointer?: LogPointer): LogPointer {
		if (!pointer) {
			return {
				index: this.data[0],
				generation: this.data[1],
				corralIndex: this.corral[0],
				corralGeneration: this.corral[1],
			};
		}
		pointer.index = this.data[0];
		pointer.generation = this.data[1];
	...
	copyPointer(pointer: LogPointer): LogPointer {
		return {
			...
			corralIndex: pointer.corralIndex,
			corralGeneration: pointer.corralGeneration,
		};
	}
	...
	processSince(
		startPointer: LogPointer,
		endPointer?: LogPointer,
		...
		if (startPointer.generation === endGeneration) {
			if (startPointer.index < endIndex) {
				result = [
					this.data,
					startPointer.index + HEADER_LENGTH,
				...
				if (corralHasNewEntries) {
					result = [
						this.corral,
						startPointer.corralIndex + HEADER_LENGTH,
	...
	countSince(startPointer: LogPointer, endPointer?: LogPointer): number {
		...
		DEBUG: if (this.corral[0]) {
			throw new InternalError(
				`Should commit log ${this.configParamName} before counting`,
			);
		}
	...
	private checkPointers(
		...
		if (endPointer) {
			...
			DEBUG: {
				if (
					startPointer.index > endPointer.index &&
	...
	private checkPointer(pointer: LogPointer): void {
		...
		DEBUG: {
			...
			if (
				...
				throw new InternalError(
					`Pointer past end of corral area in log ${this.configParamName}`,
				);
			}
		}
	}
