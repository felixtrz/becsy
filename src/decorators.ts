...
export function field(
	...
	return function (target: any, name: string): void {
		const options: PropOptions<unknown> =
			'type' in practicalOptions
				? practicalOptions
				: { type: practicalOptions };
...
function makeVectorDecorator(type: Type<number>) {
	const fn: VectorDecorator = addFieldSchema.bind(null, {
		type,
	}) as VectorDecorator;
	fn.vector =
		(elements: number | string[], Class?: new () => any) =>
		(target: any, name: string) => {
			addFieldSchema(
				{ type: Type.vector(type, elements, Class) },
				target,
				name,
			);
		};
...
export function component(
	arg: ComponentType<Component> | ComponentOptions | ComponentEnum,
	options?: ComponentOptions,
...
export function system(
	arg: SystemType<any> | SystemGroup | ScheduleFn | undefined,
	scheduler?: ScheduleFn,
