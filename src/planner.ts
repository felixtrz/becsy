...
export class Planner {
	...
	organize(): void {
		...
		for (const group of this.groups) {
			group.__plan = this.dispatcher.threaded
				? new ThreadedPlan(this, group)
				: new SimplePlan(this, group);
		}
	}
	...
	private mergeAccessorsOfUnsharedComponentTypes(): void {
		for (const componentType of this.dispatcher.registry.types) {
			...
			let lane = componentType.options?.restrictedToMainThread
				? this.mainLane!
				: this.createLane();
	...
	private reduceLanes(maxNumLanes: number): void {
		...
		for (let i = 1; i < this.lanes.length - 1; i++) {
			// don't merge into lane 0 unless necessary
			...
			for (let j = i + 1; j < this.lanes.length; j++) {
				...
				pairs.push({
					laneA,
					laneB,
					independence: this.computeIndependence(laneA, laneB),
				});
			}
		}
		...
		while (numLanes > maxNumLanes) {
			...
			const discardedLane =
				combinedLane === tangledPair.laneA
					? tangledPair.laneB
					: tangledPair.laneA;
			numLanes -= 1;
			if (numLanes > maxNumLanes) {
				pairs = pairs.filter((pair) => {
					...
					if (pair.laneA === combinedLane || pair.laneB === combinedLane) {
						pair.independence = this.computeIndependence(
							pair.laneA,
							pair.laneB,
						);
					}
	...
	private computeIndependence(laneA: Lane, laneB: Lane): number {
		return Math.min(
			this.computeIndependentWeight(laneA, laneB),
			this.computeIndependentWeight(laneB, laneA),
		);
	}
	...
	private computeIndependentWeight(lane: Lane, otherLane: Lane): number {
		...
		for (const system of lane.systems) {
			...
			for (const otherSystem of otherLane.systems) {
				if (
					!this.graph.hasPath(system, otherSystem) &&
					!this.graph.hasPath(otherSystem, system)
				) {
	...
	private pruneEmptyLanes(): void {
		this.lanes = this.lanes.filter(
			(lane) => lane.id === 0 || lane.systems.length,
		);
