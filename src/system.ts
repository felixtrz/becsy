...
export abstract class System {
	...
	schedule(buildCallback: (s: ScheduleBuilder) => void): Schedule {
		...
		CHECK: if (this.__scheduleBuilder) {
			throw new CheckError(
				`Attempt to define multiple schedules in system ${this.name}`,
			);
		}
	 ...
	 */
	start<CoFn extends CoroutineFunction>(
		coroutineFn: CoFn,
		...args: Parameters<CoFn>
	): Coroutine {
...
export class SystemBox {
	...
	private declare hasTransientQueries: boolean;
	private declare ranQueriesLastFrame: boolean;
	private declare shapeLogPointer: LogPointer;
	private declare writeLogPointer?: LogPointer;
	private declare state: RunState;
	declare readonly stats: SystemStats;
	declare readonly attachedSystems: (SystemBox | undefined)[];
	declare readonly singletonComponentDefs: (
		| ComponentType<any>
		| Record<string, unknown>
	)[];
	private declare singletonStandingWrites: ComponentType<any>[];
	private declare propsAssigned: boolean;
	declare lane?: Lane;
	...
	constructor(
		...
		STATS: this.stats = dispatcher.stats.forSystem(
			system.constructor as SystemType<any>,
		);
		this.attachedSystems = this.system.__attachPlaceholders!.map(
			(placeholder) => this.dispatcher.systemsByClass.get(placeholder.type),
		);
		this.singletonComponentDefs = this.system.__singletonPlaceholders!.flatMap(
			(placeholder) => {
				return placeholder.initialValues
					? [placeholder.type, placeholder.initialValues]
					: [placeholder.type];
			},
		);
		this.singletonStandingWrites = this.system
			.__singletonPlaceholders!.filter(
				(placeholder) => placeholder.access === 'write',
			)
	...
	assignProps(props: Record<string, unknown>): void {
		if (this.propsAssigned) {
			throw new CheckError(
				`System ${this.name} has multiple props assigned in world defs`,
			);
		}
	...
	buildQueries(): void {
		...
		this.hasNegativeQueries =
			!!this.shapeQueriesByComponent[this.dispatcher.registry.Alive.id!];
		this.hasWriteQueries = !!this.writeQueries.length;
		this.hasTransientQueries = this.shapeQueries.some(
			(query) => query.hasTransientResults,
		);
	}

	buildSchedule(): void {
		const staticScheduler = (this.system.constructor as SystemType<any>)
			.__staticScheduler;
	...
	finishConstructing(): void {
		...
		this.singletonStandingWrites = this.singletonStandingWrites.filter(
			(type) => type.__binding!.trackedWrites,
		);
	}
	...
	replacePlaceholders(): void {
		...
		for (const prop in this.system) {
			...
			if (value instanceof AttachPlaceholder) {
				...
				const targetSystem =
					this.dispatcher.systemsByClass.get(targetSystemType);
		...
		if (this.dispatcher.singleton) {
			Object.defineProperty(this.system, 'singleton', {
				value: this.dispatcher.singleton,
			});
		}
	}
	...
	private runQueries(): void {
		...
		const shapesChanged = this.dispatcher.shapeLog.hasUpdatesSince(
			this.shapeLogPointer,
		);
		const writesMade =
		...
		if (
			shapesChanged ||
			writesMade ||
			(this.hasTransientQueries && ranQueriesLastFrame)
		) {
	...
	private __updateShapeQueries(): void {
		...
		let queries: QueryBox[] | undefined,
			runLength = 0;
		let log: Uint32Array | undefined,
			...
			for (let i = startIndex!; i < endIndex!; i++) {
				...
				DEBUG: if (entry & (2 ** 31)) {
					throw new InternalError(
						'Trying to process run header as entry in shape log',
					);
				}
	...
	private __updateWriteQueries(): void {
		...
		let log: Uint32Array | undefined,
			...
			endIndex: number | undefined;
		while (true) {
			[log, startIndex, endIndex] = writeLog.processSince(
				this.writeLogPointer!,
			);
			if (!log) break;
			if (runLength && !queries) {
				startIndex! += runLength;
			...
			for (let i = startIndex!; i < endIndex!; i++) {
				...
				DEBUG: if (entry & (2 ** 31)) {
					throw new InternalError(
						'Trying to process run header as entry in write log',
					);
				}
				for (let j = 0; j < queries.length; j++) {
					queries[j].handleWrite(
						entityId,
						componentFlagOffset!,
						componentFlagMask!,
					);
