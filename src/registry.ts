import {
	type ComponentType,
	assimilateComponentType,
	defineAndAllocateComponentType,
	type ComponentId,
...
export class EntityPool {
	...
	borrow(id: EntityId): Entity {
		...
		if (!entity) {
			entity = this.borrowed[id] =
				this.spares.pop() ?? new EntityImpl(this.registry);
			entity.__id = id;
		}
		return entity;
	}
...
type AllocationItem = {
	typeOrEnum: ComponentType<Component> | ComponentEnum;
	size: number;
};
...
export class Registry {
	...
	constructor(
		...
		this.staleShapes = new ShapeArrayClass(
			'registry.staleShapes',
			this.numShapeBits,
			maxEntities,
			dispatcher.buffers,
		);
		this.removedShapes = new ShapeArrayClass(
			'registry.removedShapes',
			this.numShapeBits,
			maxEntities,
			dispatcher.buffers,
		);
	...
	initializeComponentTypes(): void {
		...
		while (this.allocationItems.length) {
			...
			if (item.typeOrEnum instanceof ComponentEnum) {
				...
				enumeration.__binding = {
					shapeOffset: shapeSpec.offset,
					shapeMask: shapeSpec.mask,
					shapeShift: shift,
				};
				for (const type of enumeration.__types) {
					assimilateComponentType(
						typeId++ as ComponentId,
						type,
						shapeSpec,
						this.dispatcher,
					);
		...
		DEBUG: {
			...
			if (
				!(
					aliveBinding.shapeOffset === 0 &&
					aliveBinding.shapeMask === 1 &&
					aliveBinding.shapeValue === 1
				)
			) {
				throw new InternalError(
					'Alive component was not assigned first available shape mask',
				);
			}
		}
	}
	...
	private prepareComponentTypesAndEnums(): AllocationItem[] {
		...
		for (const type of this.types) {
			if (!type.name) {
				Object.defineProperty(type, 'name', {
					value: `Anonymous_${anonymousTypeCounter++}`,
				});
			}
			CHECK: if (!type.__internal) {
				if (typeNames.has(type.name)) {
					throw new CheckError(
						`Multiple component types named ${type.name}; names must be unique`,
					);
				}
		...
		for (const enumeration of this.enums) {
			CHECK: if (enumeration.__types.length > 2 ** 31) {
				throw new CheckError(
					`Too many types in enum: ${enumeration.__types.length}`,
				);
			}
			pool.push({
				// +1 for the implicit null value of every enum
				typeOrEnum: enumeration,
				size: Math.ceil(Math.log2(enumeration.__types.length + 1)),
			});
			for (const type of enumeration.__types) {
				CHECK: if (enumTypes.has(type)) {
					throw new CheckError(
						`Component type ${type.name} is a member of more than one enum`,
					);
				}
	...
	private removeBiggestNoLargerThan(
		maxSize: number,
		...
		for (let i = 0; i < initialComponents.length; i++) {
			...
			CHECK: {
				...
				if (type.enum) {
					...
					throw new CheckError(
						`Duplicate ${type.name} component when creating entity`,
					);
				}
			}
				...
				try {
					...
					if (!SYSTEM_ERROR_TYPES.includes(e.constructor)) {
						const systemSuffix = system
							? ` after system ${system.name} executed`
							: '';
						const componentNames =
							this.types
								.filter(
									(type) =>
										type !== this.Alive && this.hasShape(entityId, type, false),
								)
								.map((type) => type.name)
								.join(', ') || 'none';
						e.message =
	...
	private processRemovalLog(): void {
		...
		STATS: {
			this.dispatcher.stats.maxLimboComponents = this.removalLog.countSince(
				this.removalLog.copyPointer(this.oldRemovalPointer),
			);
		}
		while (true) {
			[log, startIndex, endIndex] = this.removalLog.processSince(
				this.oldRemovalPointer,
				this.prevRemovalPointer,
			);
			if (!log) break;
			for (let i = startIndex!; i < endIndex!; i++) {
				...
				if (
					!this.shapes.isSet(entityId, type) &&
					!this.removedShapes.isSet(entityId, type)
				) {
	...
	private invalidateDeletedHeldEntities(): void {
		...
		let entityId;
		while (
			(entityId = this.entityIdPool.peekSinceMark(index++)) !== undefined
		) {
	...
	hasShape(
		...
		allowRecentlyDeleted: boolean,
	): boolean {
		if (this.shapes.isSet(id, type)) return true;
		if (
			allowRecentlyDeleted &&
			this.includeRecentlyDeleted &&
	...
	matchShape(
		...
		trackingMasks?: TrackingMask[],
		negativeMask?: number[],
		negativeTypes?: ComponentType<any>[],
	): boolean {
		if (
			positiveMask &&
			positiveValues &&
			!this.shapes.match(id, positiveMask, positiveValues)
		) {
			return false;
		}
		...
		if (negativeTypes) {
			for (const type of negativeTypes)
				if (this.shapes.isSet(id, type)) return false;
		}
		if (trackingMasks) {
