...
import {
	ArrayEntityList,
	type EntityList,
	PackedArrayEntityList,
} from './datatypes/entitylist';
...
const transientFlavorsMask =
	...
	QueryFlavor.removed |
	QueryFlavor.changed |
	QueryFlavor.addedOrChanged |
	QueryFlavor.changedOrRemoved |
...
export class QueryBox {
	results: Partial<Record<QueryFlavorName, EntityList>> & {
		current?: PackedArrayEntityList;
	} = {};
	flavors = 0;
	...
	complete(): void {
		...
		this.hasMatchTracking = Boolean(
			this.withAnyRecords?.some((record) => record.lastMatches),
		);
		CHECK: {
			if (this.withMask && this.withoutMask) {
				const minLength = Math.min(
					this.withMask.length,
					this.withoutMask.length,
				);
			...
			const hasTrackers =
				!!this.trackWritesMask ||
				this.withAnyRecords?.some((item) => item.lastMatches);
			if (this.hasChangedResults && !hasTrackers) {
				throw new CheckError(
					`Query for changed entities must track at least one component`,
				);
			}
			if (!this.hasChangedResults && hasTrackers) {
				throw new CheckError(
		...
		if (this.flavors & QueryFlavor.current) {
			this.results.current = new PackedArrayEntityList(
	...
	private allocateResult(name: TransientQueryFlavorName): void {
		const dispatcher = this.system.dispatcher;
		this.results[name] = new ArrayEntityList(
			dispatcher.registry.pool,
			this.orderBy,
		);
	}
	...
	handleWrite(
		id: EntityId,
		componentFlagOffset: number,
		componentFlagMask: number,
	): void {
		if (
...
export class QueryBuilder {
	...
	withAny(...types: (ComponentType<any> | ComponentEnum)[]): this {
		CHECK: for (const type of types) {
			if (typeof type === 'function' && type.enum) {
				throw new CheckError(
					`Cannot use enum types in a withAny clause: ${type.name}`,
				);
			}
		}
		this.set(this.__system.accessMasks.read, types);
		...
		const mask: number[] = [];
		this.__query.withAnyRecords.push({
			mask,
	...
	get trackMatches(): this {
		...
		this.__query.withAnyRecords![
			this.__query.withAnyRecords!.length - 1
		].lastMatches = [];
		return this;
	}
	...
	private set(
		mask: MaskKind | number[] | undefined,
		types?: (ComponentType<any> | ComponentEnum)[],
	): void {
		if (!mask) return;
		CHECK: if (types) {
		...
		this.__lastWasWithAny =
			this.__query.withAnyRecords?.some((item) => item.mask === mask) ?? false;
		...
		const shapeMask =
			mask === this.__query.withMask ||
			mask === this.__query.withoutMask ||
			this.__lastWasWithAny;
		const trackMask = mask === this.__query.trackWritesMask;
		const map = readMask
			? this.__system.dispatcher.planner.readers!
			: writeMask || createMask || updateMask
			? this.__system.dispatcher.planner.writers!
			: undefined;
		for (const type of types) {
			CHECK: {
				if (
					!isMaskFlagSet(this.__system.accessMasks.write!, type) &&
					((readMask &&
						isMaskFlagSet(this.__system.accessMasks.create!, type)) ||
						(createMask &&
							isMaskFlagSet(this.__system.accessMasks.read!, type)))
				) {
					throw new CheckError(
...
export class Query {
	__results: Partial<Record<QueryFlavorName, EntityList>> & {
		current?: PackedArrayEntityList;
	};
	__systemName: string;
