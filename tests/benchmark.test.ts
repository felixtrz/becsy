...
describe('benchmarks', () => {
	test('packed1', async () => {
		...
		const world = await World.create({
			maxEntities: count,
			maxShapeChangesPerFrame: count * 5 + 5,
	...
	test('simpleIter', async () => {
		...
		const world = await World.create({
			maxEntities: count * 4,
			maxShapeChangesPerFrame: count * 13 + 5,
			defaultComponentStorage: 'sparse',
		...
		world.build(() => {
			for (let i = 0; i < count; i++) {
				...
				world.createEntity(A, B, { value: 1 }, C, { value: 2 }, D, {
					value: 3,
				});
				world.createEntity(A, B, { value: 1 }, C, { value: 2 }, E, {
					value: 4,
				});
			}
		});
	...
	test('fragIter', async () => {
		...
		const world = await World.create({
			...
			maxShapeChangesPerFrame: count * 53,
			defaultComponentStorage: 'sparse',
	...
	test('addRemove', async () => {
		...
		const world = await World.create({
			maxEntities: count,
			maxShapeChangesPerFrame: count * 2 + 2,
			maxLimboComponents: count * 2,
			defaultComponentStorage: 'sparse',
