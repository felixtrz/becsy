...
describe('testing enum components', () => {
	...
	test('manual enum bitfield packing', async () => {
		World.create({ defs: [state, state2] });
				...
				class Finder extends System {
					...
					execute() {
						...
						for (const entity of this.entities.current)
							message += entity.read(Start).value;
					}
				},
			],
		});
		...
		const world = World.create({
			defs: [
				class Finder extends System {
					entities = this.query(
						(q) =>
							q.changed.withAny(state).trackMatches.and.withAny(state2)
								.trackMatches,
					);
	...
	test('enforce mutal exclusion on create', async () => {
		const world = World.create();
		expect(() => world.createEntity(Start, Middle, End)).toThrowError(
			CheckError,
		);
	});
	...
	test('reject type member of multiple enums', async () => {
		expect(() =>
			World.create({ defs: [World.defineEnum(Start, End)] }),
		).toThrow(CheckError);
	});
});
