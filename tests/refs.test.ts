...
const componentTypes = [
	...
	TypeDest,
	GlobalDest,
	GlobalDestWithStales,
];

class IncrementTargetedDests extends System {
	originEntities = this.query(
		(q) =>
			q.current.with(Origin).and.using(PreciseDest, TypeDest, GlobalDest).write,
	);

	multiOriginEntities = this.query(
		(q) =>
			q.current.with(MultiOrigin).and.using(PreciseDest, TypeDest, GlobalDest)
				.write,
	);
...
class IncrementTargeters extends System {
	...
	typeEntities = this.query(
		(q) => q.current.with(TypeDest).and.using(Origin, MultiOrigin).write,
	);
	globalEntities = this.query(
		(q) => q.current.with(GlobalDest).and.using(Origin, MultiOrigin).write,
	);
...
class UpdateTargets extends System {
	originEntities = this.query(
		(q) => q.current.with(Origin).write.using(GlobalDest).update,
	);
...
class Count extends System {
	private readonly items: {
		[key: string]: { type: ComponentType<any>; query: Query };
	} = Object.fromEntries(
...
describe('follow forward references', () => {
	test('refs', async () => {
		...
		world.build((sys) => {
			...
			sys.createEntity(Origin, { target: d1 }, MultiOrigin, {
				target1: d2,
				target2: d3,
			});
		});
...
describe('backrefs storage variants', () => {
	...
	test('large entities list, with index', async () => {
		...
		world.build((sys) => {
			...
			const origins = [];
			for (let i = 0; i < 110; i++)
				origins.push(sys.createEntity(Origin, { target: d1 }));
