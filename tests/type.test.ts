...
class Big {
	...
	@field.int16.vector(['x', 'y', 'z'], Vector3)
	declare vectorWithClass: Vector3;
...
describe('getting and setting fields of various types', () => {
	...
	test('vectorWithClass', async () => {
		...
		world.build((system) => {
			const entity = system.createEntity(Big, {
				vectorWithClass: { x: 1, y: 2, z: 3 },
			});
			const entity2 = system.createEntity(Big, {
				vectorWithClass: { x: 5, y: 9, z: -2 },
			});
	...
	test('dynamicString', async () => {
		await testReadWrite('dynamicString', [
			'',
			'a',
			'foo',
			'foobarbazqux12',
