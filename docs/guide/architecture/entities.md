...
Keeping these two modes distinct also makes it clear how a system is acting on components, and allows Fecsy's scheduler to automatically parallelize system execution without needing to use expensive and error-prone locks.
...
Removing a component makes it disappear from the entity immediately, but Fecsy actually keeps it around until the end of the next frame. This is done so that every system that needs to react to the removal gets a chance to access the data of removed components. You can access recently removed components like this:
...
class Deleter extends System {
	constructor() {
		// Note the usingAll.write below, which grants write entitlements on all component types.
		this.entities = this.query(
			(q) => q.current.with(ToBeDeleted).usingAll.write,
		);
	}
